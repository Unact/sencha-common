 ...
 *
 * 3 - Print!
 * Ext.lib.grid.Printer.mainTitle = 'Your Title here'; //optional
 * Ext.lib.grid.Printer.print(grid);
 *
 ...
 *
 */
Ext.define('Ext.lib.grid.Printer', {
    ...
    statics: {
        ...
        print: function(grid, columnsToExclude, bottomText) {
                ...
                for (var key in item.data) {
                    ...
                    Ext.each(columns, function(column, col) {
                        if (column!=null && column.dataIndex == key) {
                            ...
                            value = column.renderer ? column.renderer(value, meta, item, row, col, grid.store, grid.view) : value;
                        }
                    }, this);
            ...
            // добавление в печатную форму строки итога
            var summaryFeatureIndex = -1;
            if (grid.view && grid.view.features) {
                Ext.each(grid.view.features, function(item, i) {
                    if (item.ftype === 'summary')
                    summaryFeatureIndex = i;
                }, this);
            }
            if (summaryFeatureIndex !== -1) {
                var summaryRecord = grid.view.features[summaryFeatureIndex].summaryRecord;
                var convertedData = {};
                Ext.each(columns, function(column, col) {
                    // коллонка суммируемая, либо у нее есть соответсвующий отрисовщик
                    if (column && (column.summaryType || column.summaryRenderer)) {
                        var key = column.dataIndex;
                        var value = column.summaryRenderer ? column.summaryRenderer() : summaryRecord.data[key];
                        value = column.renderer ?
                            column.renderer(grid, value, {}, summaryRecord, 0, 0, grid.store, grid.view) : value;
                        convertedData[key] = value;
                    }
                });
                data.push(convertedData);
            ...
            if(Ext.isArray(this.extraCSS)){
                for(var i=0; i<this.extraCSS.length; i++){
                    extraCSSLinks += '<link href="' + this.extraCSS[i] + '" rel="stylesheet" type="text/css"/>';
                }
            }
